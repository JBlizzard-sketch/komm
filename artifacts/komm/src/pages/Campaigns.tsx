...
import { Plus, Send, Clock, CheckCircle2, XCircle, FileText, MoreHorizontal, Trash2 } from "lucide-react";
    ...
    >
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="icon" className="w-7 h-7 opacity-0 group-hover:opacity-100 shrink-0" data-testid={`campaign-menu-${campaign.id}`}>
...
export default function Campaigns() {
  ...
  const { data, isLoading } = useListCampaigns(
    ...
    {
      query: {
        queryKey: getListCampaignsQueryKey({ status: statusFilter !== "all" ? statusFilter as "draft" | "scheduled" | "sending" | "sent" | "failed" : undefined, channel: channelFilter !== "all" ? channelFilter as "sms" | "whatsapp" | "email" : undefined, page: 1, limit: 50 }),
      },
    }
  );
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">Create your first campaign to start reaching your members.</p>
              <Link href="/campaigns/new">
